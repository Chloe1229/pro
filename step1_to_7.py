...
st.session_state.step == 7 인 경우 :
 ...
 에 없는 경우 :   
    ...
    "step7_rows" 가 st.session_state 에 없는 경우 :
        ...
        zip 파일, xml.etree.ElementTree를 ET
 로 가져오기
        def _load_rows ():
 
            zf = zipfile.ZipFile( "step7_data.xlsx" )
            zf.open ( "xl/sharedStrings.xml" )을 f 로 사용하여 :
                공유 = [t.text 가 None 이 아닌 경우 t.text , None else "" for t in ET.parse(f).findall( './/{http://schemas.openxmlformats.org/spreadsheetml/2006/main}t' )]
     
            zf.open ( "xl/worksheets/sheet1.xml" )을 f 로 열어 주세요 :
                루트 = ET.parse(f).getroot()
            ns = '{http://schemas.openxmlformats.org/spreadsheetml/2006/main}'
            rows_xml = root.findall( f'.// {ns} 행' )
            헤더 = [shared[ int (c.find( f" {ns} v" ).text)] c.get( 't' )== 's'인 경우 그렇지 않은 경우 c.find( f" {ns} v" ).text for c in rows_xml[ 0 ].findall( f' {ns} c' )]
 
            기록 = []
            rows_xml[ 1 :] 의 행 에 대해 :
                셀 = [shared[ int (c.find( f" {ns} v" ) .text)] c.get ( 't' )== 's' 이면 그렇지 않고 c.find( f" {ns} v" ) .text c.find ( f" {ns} v" ) 가 None 이 아니면 '' for c in row.findall( f' {ns} c' )]
